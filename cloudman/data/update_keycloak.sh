...
# Add CloudMan client
curl -k -X POST -H "Content-Type: application/json" -H "Authorization: bearer $token" https://{{ .Values.global.domain }}/auth/admin/realms/master/clients -d '{"clientId":"cloudman","rootUrl":"https://{{ .Values.global.domain }}/cloudman","adminUrl":"https://{{ .Values.global.domain }}/cloudman","surrogateAuthRequired":false,"enabled":true,"redirectUris":["https://{{ .Values.global.domain }}/*"],"webOrigins":["https://{{ .Values.global.domain }}/"],"notBefore":0,"bearerOnly":false,"consentRequired":false,"standardFlowEnabled":true,"implicitFlowEnabled":false,"directAccessGrantsEnabled":true,"serviceAccountsEnabled":false,"publicClient":true,"frontchannelLogout":false,"protocol":"openid-connect","attributes":{},"authenticationFlowBindingOverrides":{},"fullScopeAllowed":true,"nodeReRegistrationTimeout":-1,"defaultClientScopes":["web-origins","role_list","profile","roles","email"],"optionalClientScopes":["address","phone","offline_access","microprofile-jwt"],"access":{"view":true,"configure":true,"manage":true},"authorizationServicesEnabled":""}'
